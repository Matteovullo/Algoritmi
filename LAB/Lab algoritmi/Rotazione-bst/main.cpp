...
private:
    ...
    void rotation(Rotations way, Node<H> *node)
    {
        ...
        parent != NULL ? (node == parent->getRight() ? parent->setRight(child) : parent->setLeft(child))
                       : (void)(root = child);
...
public:
    ...
    BSTree *remove(H key)
    {
        ...
        {
            parent != NULL ? (parent->getRight() == node ? parent->setRight(NULL) : parent->setLeft(NULL))
                           : (void)(root = NULL);
        ...
        else if (node->getRight() != NULL && node->getLeft() == NULL || node->getRight() == NULL && node->getLeft() != NULL)
        {
            ...
            parent != NULL ? (parent->getRight() == node ? parent->setRight(child) : parent->setLeft(child))
                           : (void)(root = child);
